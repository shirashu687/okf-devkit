...
検証・報告・引継ぎの共通手順は設置済みだが、full retrospective（retro）の詳細手順は未設置である。検証コマンドと制約は上記2文書を正本とし、retroが未設置であることを完了報告で区別する。
...
full retrospective（retro）の実行条件は仕様で定めるが、自前の詳細手順はまだ設置していない。トリガーがある場合は、未設置であることと必要な次の一手を報告する。
