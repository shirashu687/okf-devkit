...
着手前に [config.md](../project/config.md) で実際の配置・コマンド・上流との読み替えを、[requirements.md](policy/requirements.md) で守る制約・確認方法・強制の限界を確認する。採用名と更新方法は [skill-profile.md](../project/skill-profile.md) を参照する。
...
検証・報告・引継ぎの共通詳細手順とretroゲート手順は未設置。現時点の検証コマンドと制約は上記2文書を使い、詳細手順の設置済みとは扱わない。
