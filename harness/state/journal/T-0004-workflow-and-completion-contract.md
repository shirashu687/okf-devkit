...
| project-required-current-head-rerun | はい | 成功 | target repository / `c6b91e7`（検証時点。結果記録commit `4cb39dc`） | `.venv/Scripts/python.exe tests/run_all.py` | 現在HEADの全テストが成功 | terminal output: `Ran 155 tests`; failures 0 / errors 0 / skipped 0 | T-0005外部変更とT-0004完了記録後の現HEADで再実行。前回失敗と153件時点は履歴として保持 |
